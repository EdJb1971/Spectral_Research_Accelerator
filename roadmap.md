...
`VERIFICATION.md`; `architecture.md` Section 7 holds the full defect ledger (D1-D88, of which **85 fixed, 1 partial (D18), 2 open (D84, D85)**).
...
7.  **Publish the derivation.** Decorrelation lengths, ESS, attenuation, minimum detectable effect
    ...
    *Consequence:* in the real gate role a FAIL now survives only where the derived record says
    ...
    every branch is pinned, but **no real gate has run**, so no atmospheric absence has been
    adjudicated by it and the frozen crop's own attenuation curve does not yet exist.
