...
| D71 | `data_layer/era5_overlap.py:_atomic_write_new`, `analysis_engine/gate_run.py:443`, `analysis_engine/gate_campaign.py:372`, `forecasting/evaluation_run.py:198`, `forecasting/evaluation_job.py:100` | **Every immutable-evidence publication path fails on the drive the repository lives on.** All five publish a receipt by writing a `.tmp` beside the target and then calling `os.link(temporary, target)`. The hard link is doing two jobs at once and both matter: it is atomic, and it raises `FileExistsError` rather than overwriting, which is what makes a published receipt immutable - the guarantee D51 was created to establish after the gate was found trusting an unauthenticated manifest string. **exFAT does not support hard links.** `C:` on this machine is NTFS and `D:`, which holds the repository and therefore `DEFAULT_CACHE_DIR` (`data/zarr_cache`), is exFAT, so `os.link` raises `OSError: [WinError 1] Incorrect function` and `era5_overlap` converts it into *cannot atomically publish ERA5 overlap receipt*. The receipt path is derived from `manifest_path(spec, cache_dir)`, so in ordinary use it lands inside the repo and the publication cannot succeed. This was invisible because the tests wrote to `basetemp`, which defaulted to `C:` - the one NTFS volume - so the suite exercised these paths only on a filesystem the deployment does not use. Exposed by pinning `basetemp` to the repo drive after `C:` filled; two `test_cds_source` tests then failed for the real reason. **Not fixed here, because the repair is a semantics decision, not a typo:** `os.replace` is atomic on exFAT but silently overwrites, which discards the immutability half; `os.rename` refuses an existing target on Windows but overwrites on POSIX, so it is not portable; and an existence check followed by a rename reintroduces the race the link was chosen to avoid. Whatever is chosen must keep both properties on both filesystems and be tested on each. | **OPEN** - blocks any gate or evaluation run whose evidence is published to an exFAT volume |
...
| `test_zarr_source.py` | 60 | R13 geometry, chunk-hostility, byte counting, streaming content identity, exact chunk-bounded frame reader, cache/provenance round trip, NetCDF engine and HTTP surface; D67 coordinate-only costing that succeeds even when the data-selection path raises `MemoryError` |
