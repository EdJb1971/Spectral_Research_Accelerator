...
| D67 | `data_layer/zarr_source.py:assess_access_pattern` | **The cost estimator cannot cost the store it was most needed for.** Probing GLORYS for TG12.1 raised `MemoryError` inside dask's `slice_slices_and_integers` while building the graph for a three-year regional crop of a 12227 x 50 x 2041 x 4320 array. The function is chunk arithmetic that transfers nothing, so nothing about the *answer* requires materialising a graph that large; the cost is incurred constructing a dask slice purely in order to measure it. This is the function behind the Acquire tab's **Inspect** button, which exists so a researcher learns a crop is unaffordable *before* the download — so the failure lands exactly where the warning was supposed to be. Every amplification figure recorded for TG12.1 was therefore computed directly from the probed chunk shapes instead, which is what this function should be doing. Found by pointing it at the first store larger than ERA5; no ERA5 store is big enough to trigger it. | **OPEN** |
