...
def select_device(prefer: Optional[str] = None, run_idx: int = 0) -> torch.device:
    """Choose a device: explicit request, then env override, then CUDA -> MPS -> CPU.

    ``run_idx`` spreads concurrent runs across multiple CUDA devices. It is only meaningful
    ...
    if requested:
        ...
        if requested.startswith("cuda"):
            if not torch.cuda.is_available():
                raise RuntimeError(
                    "%s=%r was requested but CUDA is not available on this machine. "
